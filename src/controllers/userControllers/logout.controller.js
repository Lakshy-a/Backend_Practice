...
const logoutController = async (req, res) => {
  ...
  try {
    ...
    if (!accessToken) return errorHandler(res, 401, "You are not logged in...");
    ...
    if (!userId) return errorHandler(res, 401, "Invalid access token");
    ...
    errorHandler(res, 200, "User logged out successfully...");
  } catch (error) {
    console.log(error);
    res
      .status(400)
